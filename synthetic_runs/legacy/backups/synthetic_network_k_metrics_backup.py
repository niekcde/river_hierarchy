...
def compute_metrics(
    ...
    rows = []
    network_ids = network_ids[0:500]
    for i, nid in enumerate(tqdm(network_ids, total=len(network_ids), desc="Computing K metrics"), start=1):
        try:
            net = load_network_by_id(recipes_path, int(nid))
