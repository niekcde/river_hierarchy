...
def _compute_k_value(length_m: float, width_m: float) -> float:
...
def _build_reach_graph(G: nx.MultiDiGraph):
    reach_info = {}
    node_to_out = {}
    node_to_in = {}
    for u, v, k, data in G.edges(keys=True, data=True):
        ...
        k_val = _compute_k_value(length, width)
...
def compute_q_weighted_metrics(
    ...
    """
    Compute Q-weighted K metrics between first bifurcation (B) and final confluence (C).
    Returns (metrics_dict, info_dict) or (None, {"error": msg}).
    """
    RG, reach_info, node_to_out, node_to_in = _build_reach_graph(G)
    ...
    # Flow-weighted expected cumulative travel time tau for each reach
    tau = {}
    ...
    if W <= 0:
        ...
        K_Q = sum(float(SG.nodes[r].get("w", 0.0)) * float(SG.nodes[r].get("K", 0.0))
                  for r in SG.nodes) / W
        tau_mean = sum(float(SG.nodes[r].get("w", 0.0)) * float(tau[r]) for r in SG.nodes) / W
        tau_var = sum(float(SG.nodes[r].get("w", 0.0)) * (float(tau[r]) - tau_mean) ** 2
                      for r in SG.nodes) / W

    # Dominant path by max bottleneck flow
    best_cap = -np.inf
    K_dom = np.nan
    n_paths = 0
...
def maximin_sample_ids(
    ...
    """
    Greedy maximin (farthest-point) selection on specified columns.
    Returns a list of network_id values.
    """
    if n_total <= 0:
        return []
    df = summary_df.copy()
    df = df.dropna(subset=[*cols, "network_id"])
    if df.empty:
        return []
...
def run_sampled_realizations(
    ...
    for network_id in tqdm(network_ids, total=len(network_ids), desc="Running networks"):
        ...
        # Q-weighted K metrics between first bifurcation and final confluence
        kq_row = {
