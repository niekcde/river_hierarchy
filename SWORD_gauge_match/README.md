...
4. `locate-subdaily`
   ...
   - When `outputs/subdaily_values/<KH|LA|TH>/Waterlevel/` contains locally exported MRC telemetry water-level CSVs, the downloader now prefers those local stage archives over the coarse corrected-discharge API rows and derives `15 min` discharge with the official MRC mainstream rating curves from the 2024 technical note (`Technical Note – Development and update of water level and discharge rating curves for the Mekong mainstream`, DOI `10.52107/mrc.bjv4xx`, Table 3).
...
- For KH, LA, and TH downloads, the per-country `download-subdaily --output-dir outputs/subdaily_values ...` path also checks for a sibling `Waterlevel/` directory. If present, it converts those local telemetry stage files to discharge with the official MRC 2024 rating equations and records the source CSV, equation, DOI, and any proxy-stage usage in `subdaily_download_manifest.csv`. The current Thailand manual path uses `012002 Nong Khai 2` as the local stage proxy for the official `012001 Nong Khai` discharge curve and flags that explicitly in the manifest.
